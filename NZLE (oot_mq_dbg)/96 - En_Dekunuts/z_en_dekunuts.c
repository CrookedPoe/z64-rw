/************
 * z_en_dekunuts.c faithfully rewritten by CrookedPoe <nickjs.site>
 * This is meant to be a functionally equivalent rewrite, intended to be compiled with <https://github.com/z64me/z64ovl>
 * This is working as of commit e22f22f0cdaa3c002590dc0c4557063dd854ce4b
*************/
...
const z64_collider_cylinder_init_t dekunuts_collision = {
	.base = {
        .type = COL_TYPE_UNK6
        , .flags_at = 0x00
        , .flags_ac = 0x09
...
const uint32_t data_809EABA4[] =
{
    ...
    , ICHAIN(ICHAIN_F, unk_0x4C, 2600) & ICHAIN_LAST
};
...
static void func_809E96FC(entity_t* en)
{
	...
	(en->collider).dim.height = 5;
	z_lib_vec3f_copy(&(en->actor).pos, &(en->actor).pos_init);
	(en->collider).base.flags_ac &= 0xFE;
...
static void func_809E99D8(entity_t* en)
{
	...
	if (((((en->collider).body.hit_item_ac)->toucher).flags & 0x1F824) == 0)
	{
		yaw = z_actor_math_yaw_actor(&en->actor, (en->collider).base.ac);
		(en->actor).dir.y = yaw - 0x8000;
	}
	else
		(en->actor).dir.y = (((en->collider).base.ac)->dir).y;
...
static void data_809EA5B0(entity_t* en, z64_global_t* gl)
{
	...
	vec3f_t effect_coordinates = (en->actor).pos;
	...
	if (z_skelanime_draw_table(&en->skelanime))
	{
		...
		/* Leaves */
		effect_coordinates.y = (en->actor).pos.y + 10.0f;
		...
		/* Drop Item on Death */
		z_item_drop_random(gl, &en->actor, &(en->actor).pos, 0x0030);
...
static void data_809EA1D8(entity_t* en)
{
	...
	z_lib_smooth_scale_max_s(&(en->actor).rot.y, (en->actor).rot_toward_link_y, 2, 0xE38);
}
...
static void data_809E9560(entity_t* en, z64_global_t* gl)
{
	...
	{
		z_actor_shadow_init(&(en->actor).rot, 0, &Z_SHADOW_CIRCLE, 35.0f);
		_z_skelanime_init(gl, &en->skelanime, SKL, ANIM_IDLE, en->dt_rot, en->dt_pos, (LIMB_TOTAL + 1));
		z_collider_cylinder_alloc(gl, &en->collider);
		z_collider_cylinder_init(gl, &en->collider, &en->actor, &dekunuts_collision);
...
static void data_809EA0C4(entity_t* en)
{
	...
	if (!(z_skelanime_draw_table(&en->skelanime)))
	{
	  ...
	  if (frame < 1.0f)
	    frame = 1.0f;
	      ...
	      frame = 3.0f;
	  (en->collider).dim.height = (3.0f - frame) * 12.0f + 5.0f;
	}
	else
	  func_809E96FC(en);
	...
	z_lib_smooth_scale_max_f(&(en->actor).pos.x, (en->actor).pos.x, 0.5f, 3.0f);
	z_lib_smooth_scale_max_f(&(en->actor).pos.z, (en->actor).pos.z, 0.5f, 3.0f);
}
  ...
  if (!(z_skelanime_draw_table(&en->skelanime)))
    ...
    if (!(z_skelanime_frame_index_test(&en->skelanime, 6.0f)))
      ...
      deku_nut = z_actor_spawn(
				...
				, gl
				, 0x0193
				, (z_sin_s((en->actor).rot.y)) * 23.0f + (en->actor).pos.x
				, (en->actor).pos.y + 12.0f
        , (z_cos_s((en->actor).rot.y)) * 23.0f + (en->actor).pos.z
				, (en->actor).rot.x
				, (en->actor).rot.y
				, (en->actor).rot.z
				, 0x0000
...
static void data_809EA82C(entity_t* en, z64_global_t* gl)
{
  if ((en->actor).variable != VAR_FLOWER)
    ...
    z_actor_find_bounds(gl, &en->actor, 20.0f, (en->collider).dim.radius, (en->collider).dim.height, 0x1D);
    z_collider_cylinder_update(&en->actor, &en->collider);
    if (((en->collider).base.flags_ac & 1) != 0)
...
static void data_809EA240(entity_t* en)
{
	...
	z_lib_smooth_scale_max_min_s(&(en->actor).dir.y, en->inst0198, 1, 0xE38, 0xB6);
	if (xz_vel_approx == 0)
	{
		...
		if ((bg_chk & 0x20) == 0)
		{
			if ((bg_chk & 8) == 0)
			{
				if (en->inst0195 == 0)
				{
					...
					iVar6 = ((z_actor_math_yaw_vec3f(&en->actor, &(en->actor).pos_init)) - iVar7) * 0x10000 >> 0x10;
					xz_vel_approx = -iVar6;
					if (-1 < iVar6)
					{
						xz_vel_approx = iVar6;
					...
					if (xz_vel_approx < 0x2001)
					{
						...
						en->inst0198 = z_actor_math_yaw_vec3f(&en->actor, &(en->actor).pos_init);
				}
				else
					en->inst0198 = (en->actor).rot_toward_link_y + 0x8000;
			}
			...
			en->inst0198 = z_actor_math_yaw_vec3f(&en->actor, &(en->actor).pos_init);
		xz_dir = (en->actor).dir.y;
	}
	else
		xz_dir = (en->actor).dir.y;
	bVar1 = en->inst0195;
	(en->actor).rot.y = xz_dir + -0x8000;
	if (bVar1 == 0)
	{
		fVar8 = z_actor_math_dist_xz_vec3f(&en->actor, &(en->actor).pos_init);
		if (fVar8 < 20.0f)
		{
			if (ABS((en->actor).pos.y - (en->actor).pos_init.y) < 2.0f)
			{
				(en->actor).mass = -1;
				(en->actor).xz_speed = 0.0f;
				func_809E9868(en);
...
static void data_809E9B98(entity_t* en)
{
	...
	if (frame_now < 9.0f)
		...
		frame_now = 12.0f;
	(en->collider).dim.height = (frame_now - 9.0f) * 9.0f + 5.0f;
...
static void func_809EA70C(entity_t* en, z64_global_t* gl)
{
  ...
  if (((en->collider).base.flags_ac & 2) == 0)
	{
    ...
    if ((en->actor).mass == 0x32)
      ...
      if ((damage_fx != NONE) || ((en->actor).damage != 0))
        ...
        if (damage_fx == STUN)
          ...
          if (damage_fx == FIRE)
					{
            z_effect_spawn_flame_circle(gl, &en->actor, &(en->actor).pos, 0x28, 0x32);
          }
          func_809E99D8(en);
